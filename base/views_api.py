...
from .utils.checkuser import checkdata
...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            if serializer.checkPassword(serializer.validated_data):
                account = User.objects.create_user(**serializer.validated_data)
    ...
    def create_wechat(self, request):
        res = self.verify_wechat(request)
        errorinfo = res.get('error',None)
        if errorinfo:
            return JsonResponse(res)
        try:
            account = User.objects.get(username = res['openid'])
        except ObjectDoseNotExist:
            account = User.objects.create_user(username = res['openid'], password = res['openid'])
        print(account)
        pwd = account.password
        if check_password(res['openid'],pwd):
    ...
    def register(self, request):
        print(request.POST)
        if request.POST.get('request_type','') == "Wechat":
            return self.create_wechat(request)
