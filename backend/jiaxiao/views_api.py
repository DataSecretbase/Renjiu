...
class AccountViewSet(viewsets.ModelViewSet):
	...
	if self.request.user.id != user_id:
		target_uesr = User.objects.get(id = user_id)
	...
	def dispatch(self, request):
		recent_users = User.objcets.all().order_by('-last_login')
		page = self.paginate_query(recent_users)
	...
	@list_route()
	def who_visited_my_profile(self, request, *args, **kwars):
		if request.auth is None:
			return Response({
				"status":"Unauthorized",
			"message":"This authentication has been diable."
		...
		if page is not None:
			serializer = ViewdProfileTrackingSerializer(response, many = True)
			return self.get_paginated_response(serializer.data)
